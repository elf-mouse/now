...
// import data from './data'; // Using dynamic data
  ...
  var travel = {
      ...
      for (var j = 0; j < travel.test[i].item.length; j++) {
        ...
        function message(t) {
          $('.ans')
            .eq(j)
            .on('click', function() {
              // console.log('ans', $(this), travel.rightAnswers);
              //对弹出面板设置
              ...
              setTimeout(function() {
                let tipKey = ['A', 'B', 'C', 'D'][travel.test[i].ans];
                let tipValue = travel.test[i].item[travel.test[i].ans];
                ...
                $('.sh_tip').html(`正确答案：<br>${tipKey} ${tipValue}`);
                $('.shelter').show(300);
                $('.shelter').addClass('fadeIn');
                $('.sh_back').addClass('rotateIn');
                $('.tip_btn').off('click');
                $('.tip_btn').on('click', function() {
                  // console.log('tip_btn', $(this), travel.rightAnswers);
    ...
    conclude: function() {
      ...
      setTimeout(function() {
        ...
        $('.re_p').html(
          `<b>答题完毕，答对${travel.rightAnswers}题。</b><br/>科学防护，从我做起！`
        );
